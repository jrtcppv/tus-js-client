...
class Upload {
  ...
  _createUpload() {
    ...
    var headers = this._fetchHeaders();
    if (this.options.uploadLengthDeferred) {
      headers["Upload-Defer-Length"] = 1;
    } else {
      console.log("SETTING UPLOAD LENGTH TO " + this._size);
  ...
  _resumeUpload() {
    ...
    .then(response => {
      ...
      let offset = parseInt(response.headers["Upload-Offset"], 10);
      if (isNaN(offset)) {
        this._emitError(new Error("tus: invalid or missing offset value"));
        return;
      }

      let length = parseInt(response.headers["Upload-Length"], 10);
    ...
    this._source.slice(start, end, (err, value, complete) => {
      ...
      if (this.options.uploadLengthDeferred) {
        if (complete) {
          ...
          console.log("SETTING UPLOAD LENGTH TO " + this._size + " AFTER DEFERRING");
          headers["Upload-Length"] = this._size;
        }
      }
      body = value;
